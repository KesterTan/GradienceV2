...
// Returns { instructors: [{id, name, email}], students: [{id, name, email}], creatorId }
export async function getCourseMembers(courseId: number) {
  ...
  const creatorArr = await db.select({ id: users.id, firstName: users.firstName, lastName: users.lastName, email: users.email })
    .from(users)
  ...
  if (creator && !instructorList.some(i => i.id === creator.id)) {
    instructorList = [{ id: creator.id, name: `${creator.firstName} ${creator.lastName}`, email: creator.email }, ...instructorList];
  }
